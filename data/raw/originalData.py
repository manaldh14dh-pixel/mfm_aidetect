...
#البحث عن الرموز الغريبة في النصوص:
import re
sample_dataset = merged_df_dataset['text'].sample(50).tolist()
for s in sample_dataset:
    strange_chars = re.sub(r'[a-zA-Z0-9\u0600-\u06FF\s]', '', s)
    if strange_chars.strip() != "":
        print("رموز غريبة:", strange_chars)
    else:
        print("لا يوجد رموز غريبة في هذا النص")
